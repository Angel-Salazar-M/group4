<x-layout title="Patients list">
    ...
    <div class="flex justify-between">
        ...
        <button class="border-2 bg-[#76ABD7] rounded-full mt-6 mr-6 py-2 px-10" @click="modal = !modal" :aria-expanded="modal ? 'true' : 'false'"><svg xmlns="http://www.w3.org/2000/svg"
    ...
    <div class="mt-10 flex justify-center mx-5">
        <table class="border-2 border-black py-2 px-14">
            <thead class="border-2 border-black py-2 px-14">
                ...
                <th class="border-2 border-black py-2 px-14">FECHA DE NACIMIENTO</th>
                <th class="border-2 border-black py-2 px-14">EDAD</th>
                <th class="border-2 border-black py-2 px-14">ESTADO CIVIL</th>
                <th class="border-2 border-black py-2 px-14">TIPO DE SANGRE</th>
            </thead>
            <tbody class="border-2 border-black py-2 px-14">
                @foreach ($patients as $patient)
                    <tr>
                        <td class="border-2 border-black py-2 px-14 text-center"> {{ $patient->user->name }} </td>
                        <td class="border-2 border-black py-2 px-14 text-center"> {{ $patient->user->dui }} </td>
                        <td class="border-2 border-black py-2 px-14 text-center"> {{ $patient->user->birthday }} </td>
                        ...
                        <td class="border-2 border-black py-2 px-14 text-center"> {{ $patient->blood_type }} </td>
                    </tr>
                @endforeach
            </tbody>
        </table>
